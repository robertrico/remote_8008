...
{
	/* Bring-up: pad every small frame well past the 64-byte ethernet
	 * minimum. Empirically only our minimum-size (60+4) TX frames vanish
	 * between the MAC and the LAN (ARP replies, gratuitous announces)
	 * while 300-byte DHCP/loopback frames pass -- suspect the MAC's
	 * hardware min-frame padding path. Trailer padding is protocol-legal:
	 * ARP/IP carry their own lengths. */
	if(txlen < 100) {
		memset(&txbuffer->raw[txlen], 0, 100 - txlen);
		txlen = 100;
	}
...
// Source MAC of the most recent frame handed to a UDP callback. Lets the
// Etherbone server reply to a broadcast request by unicasting straight back
// to the requester's MAC -- without ARP-resolving it, which would depend on
// the client->board unicast direction the mesh drops.
uint8_t udp_last_src_mac[6];

// Pin the TX peer directly (bypasses the ARP cache/resolve machinery).
...
// Force a fresh ARP round-trip to `ip` even if it is already cached. The
// request itself is the point: it carries our sender IP/MAC, which the
// gateway gratuitously caches -- repopulating its table for us after its
// negative-cache backoff has stopped it from ever asking again.
...
// Gratuitous ARP announce: broadcast "who has <my_ip>? tell <my_ip>" with our
// MAC as sender. Repopulates every ARP table on the segment -- without it the
// router serves the DHCP lease but, having never resolved us, drops unicast
// traffic for our address without asking again (negative-cache backoff).
void udp_announce_arp(void)
{
	struct arp_frame *tx_arp = &txbuffer->frame.contents.arp;
	int i;

	if(my_ip == 0 || my_ip == 0xffffffff)
		return;
	fill_eth_header(&txbuffer->frame.eth_header, broadcast, my_mac,
	                ETHERTYPE_ARP);
	txlen = ARP_PACKET_LENGTH;
	tx_arp->hwtype = htons(ARP_HWTYPE_ETHERNET);
	tx_arp->proto = htons(ARP_PROTO_IP);
	tx_arp->hwsize = 6;
	tx_arp->protosize = 4;
	tx_arp->opcode = htons(ARP_OPCODE_REQUEST);
	tx_arp->sender_ip = htonl(my_ip);
	for(i=0;i<6;i++)
		tx_arp->sender_mac[i] = my_mac[i];
	tx_arp->target_ip = htonl(my_ip);
	for(i=0;i<6;i++)
		tx_arp->target_mac[i] = 0;
...
static void process_udp(void)
{
	...
	/* Record the requester's MAC for ARP-free replies (see udp_set_peer). */
	memcpy(udp_last_src_mac, rxbuffer->frame.eth_header.srcmac, 6);

	if(ntohl(udp_ip->ip.dst_ip) != my_ip) {
#ifdef ETH_UDP_BROADCAST
		/* If the destination IP is not mine, check if it is a broadcast.
		 * Accept the /24 subnet-directed broadcast too (my_ip | .255):
		 * mesh WiFi forwards subnet broadcast where it drops the
		 * limited (255.255.255.255) form. */
		if((ntohl(udp_ip->ip.dst_ip) == IPTOINT(255, 255, 255, 255) ||
		    ntohl(udp_ip->ip.dst_ip) == (my_ip | 0xff)) && bx_callback) {
...
// Bring-up counters (read from main.c's stats line).
uint32_t dbg_rx_frames, dbg_rx_arp, dbg_rx_ip, dbg_rx_short;
uint32_t dbg_ip_tome, dbg_ip_bcast, dbg_ip_other;
...
static void process_frame(void)
{
	...
	dbg_rx_frames++;
	if(rxlen < (sizeof(struct ethernet_header) + ETH_TX_CRC_SIZE)) { dbg_rx_short++; return; }
	...
	if (ntohs(rxbuffer->frame.eth_header.ethertype) == ETHERTYPE_ARP) {
		...
		dbg_rx_ip++;
		{
			uint32_t dip = ntohl(rxbuffer->frame.contents.udp.ip.dst_ip);
			if(dip == my_ip)                dbg_ip_tome++;
			else if((dip & 0xff) == 0xff)   dbg_ip_bcast++;
			else                            dbg_ip_other++;
		}
		struct ip_header *hdr = &rxbuffer->frame.contents.udp.ip;
		if(hdr->version != IP_IPV4)
			return;
		/* Let broadcast through to process_udp: its ETH_UDP_BROADCAST
		 * path (bx_callback) is unreachable if this outer filter only
		 * passes dst == my_ip. Accept limited and /24 subnet-directed
		 * broadcast alongside our own address. */
		if(ntohl(hdr->dst_ip) != my_ip &&
		   ntohl(hdr->dst_ip) != 0xffffffffu &&
		   ntohl(hdr->dst_ip) != (my_ip | 0xff))
			return;
